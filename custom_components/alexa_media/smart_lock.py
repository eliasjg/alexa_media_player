...
"""
Alexa Devices Smart Lock Controller.
...
async def async_setup_platform(
    ...
    if "lock" not in (account_dict["entities"]):
        (
...
async def async_unload_entry(hass, entry) -> bool:
    ...
    account_dict = hass.data[DATA_ALEXAMEDIA]["accounts"][account]
    for device in account_dict["entities"]["lock"].values():
        await device.async_remove()
    return True


class AlexaLockDevice(LockDevice):
    ...
    def __init__(self, login) -> None:
        ...
        self._friendly_name = "Alexa Lock"
        self._state = None
